...
def snow17(time, prec, tair, lat=50, elevation=0, dt=24, scf=1.0, rvs=1,
    ...
    for i, t in enumerate(time):
        ...
        if deficit == 0:
            ait = 0
        ## ice-ground melt with a constant rate. 
        if w_i>DAYGM:
            gmwloss = (DAYGM/w_i) * w_q # water loss
            gmsloss = DAYGM # ice loss
            gmro = gmwloss + gmsloss
            w_i = w_i - gmsloss
            w_q = w_q - gmwloss

            e = e + gmro # excess water
            swe = w_i + w_q
        else: 
            gmro = w_i + w_q
            w_i = 0
        ...
        outflow[i] = e
        with open('single.txt', 'a') as f:
            print(i, ',', w_i, ',', melt, ',', e, ',', deficit, ',', ait, ',', w_q, ',', swe, ',', delta_hd_snow, ',',\
            delta_hd_t, ',', gmro, ',', m_ros, ',', m_nr, ',', fracsnow, file=f)
